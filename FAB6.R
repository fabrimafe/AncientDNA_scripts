...
divtime<-as.numeric(args[3])
#divtime<-0
#pop<-"D"
#pop<-"S_Khomani_San-1.qual2"
mylength<-as.numeric(system(paste0("cat ",input_file," | wc -l "),intern=TRUE))
mydata<-scan(input_file,what=character(),n=mylength)
...
write(paste(c(divtime,mean(res[2,]/res[1,],na.rm=TRUE),mean(res[3,]),mean(res[4,]),mean(res[5,]),mean(res[6,])),collapse=" "),file=output_file,append=TRUE)
