...
i=$3
mkdir ${mypopA}.${mypopB}
bedtools intersect -b /mnt/454/Vindija/high_cov/Manifesto/${mypopB}/chr${i}_mask.bed.gz -a <( zcat /mnt/sequencedb/gendivdata/2_genotypes/human/SGDP/release_initial/vcfs/${mypopA}.chr${i}.vcf.gz ) -sorted | awk -v OFS='\t' '{print $1,$2-1,$2}' | gzip -f > ${mypopA}.${mypopB}/${mypopA}.chr${i}.bed.gz
bedtools intersect -b /mnt/454/Vindija/high_cov/Manifesto/${mypopB}/chr${i}_mask.bed.gz -a <( zcat /mnt/454/Vindija/high_cov/genotypes/Altai/all_sites_VCF/chr${i}_mq25.vcf.gz ) -sorted | awk -v OFS='\t' '{print $1,$2-1,$2}' | gzip -f > ${mypopA}.${mypopB}/${mypopB}.chr${i}.bed.gz
...
bedtools intersect -b ${mypopA}.${mypopB}/chr${i}.mask.bed.gz -a <( zcat /mnt/sequencedb/gendivdata/2_genotypes/human/SGDP/release_initial/vcfs/${mypopA}.chr${i}.vcf.gz ) -sorted | vcf2haploidvcf 0 | grep -v '#' | myvcf2fa 0 $MYBEGIN $MYEND | sed 's/[nN]/-/g' | sed "s/>${i}/>${mypopA}/g" > ${mypopA}.${mypopB}/chr${i}.fa
bedtools intersect -b ${mypopA}.${mypopB}/chr${i}.mask.bed.gz -a <( zcat /mnt/454/Vindija/high_cov/genotypes/Altai/all_sites_VCF/chr${i}/chr${i}_mq25.vcf.gz ) -sorted | vcf2haploidvcf 0 | grep -v '#' | myvcf2fa 0 $MYBEGIN $MYEND | sed 's/[nN]/-/g' | sed "s/>${i}/>${mypopB}/g" >> ${mypopA}.${mypopB}/chr${i}.fa
