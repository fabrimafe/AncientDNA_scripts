...
MYQUALITY=${6}
TABIX=$7 #parameters that tells wheter the Manifesto is tabixindexed (1), not (0) or wheter there is no Manifesto at all (2), or no removal of regions (3)
...
echo "tabix 2"
elif [ $TABIX -eq 3 ]; then
echo "tabix 3"
zcat $MYMANIFESTO | gzip -f > $FINALBED
echo "zcat $MYMANIFESTO | gzip -f > $FINALBED"
fi
