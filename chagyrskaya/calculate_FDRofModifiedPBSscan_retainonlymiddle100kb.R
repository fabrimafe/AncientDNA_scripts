...
#output also looser FDR cutoffs
args <- commandArgs(trailingOnly = TRUE)
winsize<-as.numeric(args[1])
scan.file<-as.character(args[2])
simulations.file<-as.character(args[3])
listvalidsims<-as.character(args[4]) 
outputfolder<-as.character(args[5])

options(scipen=999)

listvalidsims<-as.numeric(c(unlist(read.table(listvalidsims))))
...
print(paste("simulations retained:",length(listvalidsims)))
#mywinsize<-"100000"
...
#print(nrow(res))
res<-res[res$CHROM %in% listvalidsims,]
#print(nrow(res))
#print(head(res))
names(res_tot)[1:2]<-c("CHROM","BIN_START")
#print(nrow(res))
res<-res[res$BIN_START>500000-51000,]
#print(head(res))
#print(nrow(res))
res<-res[res$BIN_START<500000+50000+1000-winsize,]
#print(head(res))
...
write.table(signtemp,file=paste0(myPBS,"_win",winsize,"_pvalue.bed"),quote=F,row.names=F,sep="\t")

signtemp<-res[res$pvalue<0.05,]
names(signtemp)[1]<-"#CHROM"
names(signtemp)[2]<-"INIT"
#signtemp[["BIN_START"]]<-signtemp[["BIN_START"]]-1
signtemp[["INIT"]]
signtemp[['END']]<-signtemp[['INIT']]+as.numeric(winsize)
signtemp<-signtemp[,c(1,2,ncol(signtemp),3:(ncol(signtemp)-1))]
write.table(signtemp,file=paste0(myPBS,"_win",winsize,"_pvaluesign.bed"),quote=F,row.names=F,sep="\t")
...
signtemp[['END']]<-signtemp[['INIT']]+as.numeric(winsize)
signtemp<-signtemp[,c(1,2,ncol(signtemp),3:(ncol(signtemp)-1))]
write.table(signtemp,file=paste0(myPBS,"_win",winsize,"_sign_fdr20.bed"),quote=F,row.names=F,sep="\t")

signtemp<-res[res$fdr<0.1,]
names(signtemp)[1]<-"#CHROM"
names(signtemp)[2]<-"INIT"
#signtemp[["BIN_START"]]<-signtemp[["BIN_START"]]-1
signtemp[["INIT"]]
signtemp[['END']]<-signtemp[['INIT']]+as.numeric(winsize)
signtemp<-signtemp[,c(1,2,ncol(signtemp),3:(ncol(signtemp)-1))]
write.table(signtemp,file=paste0(myPBS,"_win",winsize,"_sign_fdr10.bed"),quote=F,row.names=F,sep="\t")

signtemp<-res[res$fdr<1,]
names(signtemp)[1]<-"#CHROM"
names(signtemp)[2]<-"INIT"
#signtemp[["BIN_START"]]<-signtemp[["BIN_START"]]-1
signtemp[["INIT"]]
signtemp[['END']]<-signtemp[['INIT']]+as.numeric(winsize)
signtemp<-signtemp[,c(1,2,ncol(signtemp),3:(ncol(signtemp)-1))]
write.table(signtemp,file=paste0(myPBS,"_win",winsize,"_sign_fdr.bed"),quote=F,row.names=F,sep="\t")
