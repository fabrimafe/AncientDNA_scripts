...
setwd("/mnt/scratch/steffi/D/allele_freqs/af_high_Africa2_apes/")
...
for (i in c(as.character(1:22), "X")){
	...
	freqs = freqs[!is.na(freqs$Africa2),]
	...
	fixed_arch = freqs[arch_mean>0.975, ]
	# (B) fixed derived in Alt/Vin/Cha, fixed ancestral in Den
	fixed_nean = freqs[nean_mean>0.975 & freqs$Denisova<0.025, ]
	# (C) polymorphic in Alt/Vin/Cha/Den
	poly_arch = freqs[arch_mean>=0.025 & arch_mean<=0.975, ]
	# (D) polymorphic in Alt/Vin/Cha, fixed ancestral in Den
	poly_nean = freqs[ ( nean_mean>=0.025 & nean_mean<=0.975 ) & freqs$Denisova<0.025, ]
	...
	write_bed(fixed_arch, paste0(tmp,"/chaggo_fixed_derived_archaics_af975_chr",i,".bed"), "ref_derived")
	write_bed(fixed_nean, paste0(tmp,"/chaggo_fixed_derived_nean_af975_chr",i,".bed"), "ref_derived")
	write_bed(poly_arch, paste0(tmp,"/chaggo_poly_derived_archaics_af975_chr",i,".bed"),
		c("ref_derived","AltaiNeandertal","Chagyrskaya","Vindija33.19","Denisova"))
	write_bed(poly_nean, paste0(tmp,"/chaggo_poly_derived_nean_af975_chr",i,".bed"),
...
setwd(tmp)
system("cat chaggo_fixed_derived_archaics_af975_chr*.bed > /mnt/scratch/fabrizio/Chagyrskaya/selection/catalog_archaic_derived/chaggo_fixed_archaics_af975.bed")
system("cat chaggo_fixed_derived_nean_af975_chr*.bed > /mnt/scratch/fabrizio/Chagyrskaya/selection/catalog_archaic_derived/chaggo_fixed_nean_af975.bed")
system("cat chaggo_poly_derived_archaics_af975_chr*.bed > /mnt/scratch/fabrizio/Chagyrskaya/selection/catalog_archaic_derived/chaggo_poly_archaics_af975.bed")
system("cat chaggo_poly_derived_nean_af975_chr*.bed > /mnt/scratch/fabrizio/Chagyrskaya/selection/catalog_archaic_derived/chaggo_poly_nean_af975.bed")
