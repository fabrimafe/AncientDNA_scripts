...
cat ${INPUTFILE} | grep position | awk -v myl=${MYL} -v mychr=${MYCHR} -v FS=' ' -v OFS='\t' '{for (i=2;i<=NF;i++){print mychr,int($i*myl)+1,".","A","T",50,".",".","GT:DP:A:C:G:T:PP:GQ","0/1"}}' >> ${OUTPUTFILE} #+1 in position is just to avoid 0s
bgzip -f ${OUTPUTFILE}
