...
bedtools merge -i <(zcat $FINALBED | sort -Vu -k1,1 -k2,2n ) -sorted > $MYOUTPUTFOLDER/regions_psmc.${POP}.chr${i}.bed
...
bedtools intersect -b $FINALBED -a <(zcat /mnt/scratch/fabrizio/Vindija/header.gz $MYVCF ) -sorted | myvcf2fq_gen 0 > $MYOUTPUTFOLDER/temp.$POP.$i.fq
bedtools intersect -b $FINALBED -a <(zcat /mnt/scratch/fabrizio/Vindija/header.gz $MYVCF ) -sorted | myvcf2fq_qual 0 >> $MYOUTPUTFOLDER/temp.$POP.$i.fq
