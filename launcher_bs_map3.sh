...
filt=$1
i=$2
MYCOUNT=$( cat /mnt/scratch/fabrizio/Vindija/branchshortening/Manifesto/M0.U.map${filt}.chr${i}.ds | wc -l); if [ $MYCOUNT -lt 2 ]; then
bedtools intersect -a /mnt/454/Ust_Ishim/1_Extended_VCF/Ust_Ishim.hg19_1000g.${i}.mod.vcf.gz -b /mnt/454/Ust_Ishim/Manifeston/Ust_Ishim.${i}.Map35_${filt}.MQ30.Cov.indels.TRF.bed -sorted -header | vcf2haploidbed 0 | awk -v OFS='\t' '{print $1,$3,$4}' | ~pruefer/src/BamSNPTool/BamSNPAddMaf /mnt/sequencedb/ucsc/goldenPath/hg19/vsPanTro2/axtNet/mafnet/chr$i.hg19.panTro2.net.maf pantro2 | tr '[:lower:]' '[:upper:]' | bs_divergence 3 4 > /mnt/scratch/fabrizio/Vindija/branchshortening/Manifesto/M0.U.map${filt}.chr${i}.ds;fi
